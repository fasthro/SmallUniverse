...
namespace SU.Level
{
    public class Map : GameBehaviour
    {
        ...
        // 玩家出生点
        private MapGrid characterPointGrid;
        // 最小层
        ...
        IEnumerator GenerateMap()
        {
            ...
            for (int i = 0; i < radius; i++)
            {
                ...
                {
                    var points = GetRangePoints(layer, i);
                    for (int k = 0; k < points.Count; k++)
                    {
                        var grid = mapLayers[layer].GetGrid(points[k]);
                        if (grid != null)
                        {
        ...
        /// <returns></returns>
        private List<Vector3> GetRangePoints(int layer, int lap)
        {
            Vector3 point = characterPointGrid.position;

            List<Vector3> points = new List<Vector3>();
            // 直接返回角色出生点
            if (lap == 0)
            {
                points.Add(new Vector3(point.x, layer, point.z + lap));
            }
            else {
                Vector3[] fixedPoint = new Vector3[] {
                    new Vector3(point.x, layer, point.z + lap),
                    new Vector3(point.x + lap, layer, point.z + lap),
                    new Vector3(point.x + lap, layer, point.z),
                    new Vector3(point.x + lap, layer, point.z -lap),
                    new Vector3(point.x, layer, point.z -lap),
                    new Vector3(point.x - lap, layer, point.z - lap),
                    new Vector3(point.x - lap, layer, point.z),
        ...
        public void ParseMap()
        {
            ...
            radius = int.Parse(element.Attribute("radius"));
            ...
            foreach (SecurityElement child in element.Children)
            {
                ...
                else if (child.Tag == "character_point")
                {
                    CharacterPointParse(child);
                }
        ...
        /// </summary>
        /// <param name="element"></param>
        private void CharacterPointParse(SecurityElement element)
        {
            characterPointGrid = new MapGrid();
            characterPointGrid.layer = int.Parse(element.Attribute("layer"));
            characterPointGrid.position = new Vector3(int.Parse(element.Attribute("pos_x")), int.Parse(element.Attribute("pos_y")), int.Parse(element.Attribute("pos_z")));
            characterPointGrid.rotationAngle = new Vector3(int.Parse(element.Attribute("angle_x")), int.Parse(element.Attribute("angle_y")), int.Parse(element.Attribute("angle_z")));
        }
