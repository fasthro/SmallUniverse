...
#if UNITY_EDITOR
            if (joyFunc == JoyFunc.MoveJoy && Application.isPlaying)
            {
                float x = Input.GetAxis("Horizontal");
                float y = Input.GetAxis("Vertical") * -1f;
                ...
                if ((x != 0 || y != 0) && !m_axis)
                {
                    ...
                    m_touchStartPosition = m_virtualCenter;

                    RaiseEvent(JoyTouchEventName.Start, m_touchStartPosition);
                }
                else if (x == 0 && y == 0 && m_axis)
                {
                    m_axis = false;
                    m_fingerIndex = -1;
                    m_touchMoveing = false;

                    RaiseEvent(JoyTouchEventName.Up, Vector2.zero);
                }
                else if (m_axis)
                {
                    m_touchMovePosition.x = x * parame.radius;
                    m_touchMovePosition.y = y * parame.radius;
                    m_touchMovePosition += m_virtualCenter;
